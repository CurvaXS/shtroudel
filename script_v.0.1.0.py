...
def input_code(code, session_quantity):
    ...
    for j in range(session_quantity):
        ...
        time.sleep(2)
        parent_recaptcha = driver.find_element(
            By.XPATH,
            '//div[@class="relative inline-block w-full whitespace-normal rounded-md bg-bg-wrapper text-left align-middle text-base shadow-modal transition-[max-width] max-w-md"]'
            #By.ID, value='[id="g-recaptcha-response-2"]'
        ...
        recaptcha = parent_recaptcha.find_element(
            By.XPATH,
            './/iframe'
            #'.//div[@class="p-3 lg:p-4"]/div/div/div/iframe/'
